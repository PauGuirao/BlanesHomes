...
const VendorView = ({ onClose, onPisoClick, onSelectAgency }) => {
  ...
  const { data: vendorListData, isLoading: isLoadingVendors } = useQuery({
    queryKey: ['vendors'],
    queryFn: () => axios.get(`${API_URL}/vendedores`).then(res => res.data),
    staleTime: 1000 * 60 * 60 * 6, // 6 horas
  });

  const generalStats = vendorListData?.stats || {};
  const vendors = vendorListData?.vendedores || [];

  const [selectedVendorName, setSelectedVendorName] = useState(null);

  const { data: selectedVendorData, isLoading: isLoadingVendorDetail } = useQuery({
    queryKey: ['vendor', selectedVendorName],
    queryFn: () =>
      axios.get(`${API_URL}/vendedor/${encodeURIComponent(selectedVendorName)}`).then(res => res.data),
    enabled: !!selectedVendorName,
          ...
          .map(vendor => (
            <div key={vendor.nombre}>
              <div 
                ...
                onClick={() => {
                  if (selectedVendorName === vendor.nombre) {
                    setSelectedVendorName(null);
                    onSelectAgency(null);
                  } else {
                    setSelectedVendorName(vendor.nombre);
                    onSelectAgency(vendor.nombre);
                  }
                }}
                style={{ marginBottom: selectedVendorData?.nombre === vendor.nombre ? 0 : undefined }}
              >
              ...
              {selectedVendorData && selectedVendorData.nombre === vendor.nombre && (
                <>
                  <div className="vendor-detail-expanded">
                    <div className="vendor-detail-header">
                      <div className="vendor-detail-title">
                        {selectedVendorData.nombre}
                      </div>
                      <div className="vendor-detail-subtitle">
                        {selectedVendorData.total_propiedades} {t('vendorView.propertiesInPortfolio', 'properties in portfolio')}
                      </div>
                    </div>
                    ...
                    <div className="vendor-detail-content">
                      <div className="vendor-detail-section">
                        ...
                        <div className="stats-grid-container">
                          <div className="stat-box">
                            ...
                            <div className="stat-box-value">{selectedVendorData.precio_min.toLocaleString()}€ - {selectedVendorData.precio_max.toLocaleString()}€</div>
                          </div>
                          <div className="stat-box">
                            <div className="stat-box-label">{t('vendorView.average', 'Average')}</div>
                            <div className="stat-box-value">{selectedVendorData.precio_medio.toLocaleString()}€</div>
                          </div>
                          <div className="stat-box">
                            <div className="stat-box-label">{t('vendorView.pricePerSqm', 'Price/m²')}</div>
                            <div className="stat-box-value">{Math.round(selectedVendorData.precio_medio / selectedVendorData.metros_medio).toLocaleString()}€/m²</div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="vendor-detail-section">
                        ...
                        <div className="activity-status-container">
                          <div className={`activity-status ${selectedVendorData.nuevos_al_mes > 5 ? 'active' : 'inactive'}`}>
                            {selectedVendorData.nuevos_al_mes > 5 ? 
                        ...
                        <div className="stats-grid-container">
                          <div className="stat-box">
                            ...
                            <div className="stat-box-value">{selectedVendorData?.nuevos_al_mes ?? 0}</div>
                          ...
                          <div className="stat-box">
                            ...
                            <div className="stat-box-value">{selectedVendorData?.promedio_mensual_6meses ?? 'N/A'}</div>
                          ...
                          <div className="stat-box">
                            ...
                            <div className="stat-box-value">{selectedVendorData.tiempo_medio_mercado || 'N/A'}</div>
                      ...
                      <div className="vendor-detail-section">
                        ...
                        <div className="property-types-container">
                          {Object.entries(selectedVendorData.tipos || {}).map(([tipo, count]) => (
                            <div key={tipo} className="property-type-card">
                              ...
                              <div className="property-type-bar-container">
                                <div className="property-type-bar" 
                                  style={{ 
                                    width: `${(count / selectedVendorData.total_propiedades) * 100}%`
                                  }} 
                                />
                              </div>
                            </div>
                      ...
                      <div className="vendor-detail-section">
                        ...
                        <div className="zone-chart-container">
                          <ResponsiveContainer width="100%" height={180}>
                            <BarChart
                              data={Object.entries(selectedVendorData.zonas || {})
                                .sort((a, b) => b[1] - a[1])
                                .slice(0, 5)
                                .map(([zona, count]) => ({
                  ...
                  <div className="properties-list">
                    ...
                    {selectedVendorData.propiedades.map(piso => (
                      <div 
                        key={piso.id} 
