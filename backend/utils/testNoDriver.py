    ...
    try:
        ...
        try:
            ...
            if desc_p:
                ...
                print(f"ID {listing_id}: Descripción cruda: {descripcion_completa}")
        ...
        print(f"ID {listing_id}: Descripción extraída: {data['descripcion']}")
...
async def main():
    ...
    """
    # 1. Leer IDs del archivo
    if not os.path.exists(IDS_FILENAME):
        logging.error(f"El archivo de IDs '{IDS_FILENAME}' no existe. Crealo con un ID por línea.")
        return
    try:
        with open(IDS_FILENAME, 'r') as f:
            listing_ids = [line.strip() for line in f if line.strip().isdigit()]
        if not listing_ids:
            logging.error(f"No se encontraron IDs válidos en '{IDS_FILENAME}'. Asegúrate de que sean números.")
            return
        logging.info(f"Se leerán {len(listing_ids)} IDs desde '{IDS_FILENAME}'.")
    except Exception as e:
        logging.error(f"Error al leer el archivo '{IDS_FILENAME}': {e}")
        return

    # 2. Iniciar el navegador con nodriver
    scraped_data_list = []
