
from fastapi import APIRouter, Depends, Query, Request, HTTPException
from fastapi.responses import JSONResponse
import pandas as pd
router = APIRouter()

from deps.dependencies import (
    get_model_precio, get_features_precio,
    get_model_precio_m2, get_features_precio_m2, get_df
)

# Coordenadas por zona
zona_coords = {
    "Centre": (41.673774, 2.791094),
    "La Plantera": (41.668101, 2.774309),
    "Els Pins": (41.662931, 2.781882),
    "Els Pavos": (41.675115, 2.781567),
    "Semicentre": (41.678307, 2.787425),
    "Mont Ferrant - Sant Joan": (41.678658, 2.794806),
    "Cala Sant Francesc - Santa Cristina": (41.679922, 2.806074)
}

from auth import get_current_user

@router.post("/estimar_precio")
async def estimar_precio(data: Request, model_precio = Depends(get_model_precio), features_precio = Depends(get_features_precio), model_precio_m2= Depends(get_model_precio_m2), features_precio_m2= Depends(get_features_precio_m2), df = Depends(get_df)):
   
    # ======== 1. Cargar datos de entrada ========
    input_data = await data.json()
    df_input = pd.DataFrame([input_data])

    # ======== 2. Conversión de tipos y features iniciales ========
    for col in ['metros', 'habitaciones', 'baños']:
        df_input[col] = pd.to_numeric(df_input[col], errors='coerce')

    df_input["ratio_habitaciones_baños"] = df_input["habitaciones"] / (df_input["baños"] + 1e-5)

    zona = df_input.loc[0, "zona"]
    tipo = df_input.loc[0, "tipo"]

    if pd.isna(zona):
        raise HTTPException(status_code=400, detail="Zona es requerida")

    # Coordenadas por zona
    lat, lon = zona_coords.get(zona, (41.675, 2.792))
    df_input["latitud"] = lat
    df_input["longitud"] = lon

    # Extras
    extras_cols = ['terraza', 'jardin', 'garaje', 'piscina', 'ascensor', 'balcon']
    df_input["n_extras"] = df_input[extras_cols].sum(axis=1)

   # ======== 3. Predicción de precio por metro cuadrado ========
    df_m2_encoded = pd.get_dummies(df_input)
    df_m2_encoded[df_m2_encoded.select_dtypes(include='bool').columns] = df_m2_encoded.select_dtypes(include='bool').astype(int)

    # 3. Estimar año_construccion dinámico
    similares = df[
        (df["zona"] == zona) &
        (df["tipo"] == tipo) &
        (df["habitaciones"] == df_input.loc[0, "habitaciones"]) &
        (df["baños"] == df_input.loc[0, "baños"]) &
        (df["metros"].between(df_input.loc[0, "metros"] * 0.9, df_input.loc[0, "metros"] * 1.1)) &
        (df["año_construccion"].notnull())
    ]

    if not similares.empty:
        q1 = similares["año_construccion"].quantile(0.25)
        q3 = similares["año_construccion"].quantile(0.75)
        iqr = q3 - q1
        lower = q1 - 1.5 * iqr
        upper = q3 + 1.5 * iqr
        filtrados = similares[(similares["año_construccion"] >= lower) & (similares["año_construccion"] <= upper)]
        año_estimado = int(filtrados["año_construccion"].mean()) if not filtrados.empty else 1970
    else:
        año_estimado = 1970

    df_m2_encoded["año_construccion"] = año_estimado
    print(año_estimado)

    # Asegurar todas las columnas necesarias
    for col in features_precio_m2:
        if col not in df_m2_encoded:
            df_m2_encoded[col] = 0
    df_m2_encoded = df_m2_encoded[features_precio_m2]

    precio_m2_estimado = model_precio_m2.predict(df_m2_encoded)[0]
    df_input["precio_m2"] = float(precio_m2_estimado)

    # ======== 4. Predicción de precio total ========
    df_encoded = pd.get_dummies(df_input)
    df_encoded[df_encoded.select_dtypes(include='bool').columns] = df_encoded.select_dtypes(include='bool').astype(int)

    for col in features_precio:
        if col not in df_encoded:
            df_encoded[col] = 0
    df_encoded = df_encoded[features_precio]

    precio_estimado = model_precio.predict(df_encoded)[0]
    print(df_encoded.to_json(orient="records"))
    # ======== 5. Devolver resultado ========
    return {
        "precio_estimado": int(precio_estimado),
    }
